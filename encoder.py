...
if ":::" in raw_dump:  
    ...
    CODEC = parts[9] if len(parts) > 9 else "libx264"  
    AUDIO_CODEC = parts[10] if len(parts) > 10 else "copy"  
    WM_TYPE = parts[11] if len(parts) > 11 else "image"  
    WM_TEXT = parts[12] if len(parts) > 12 else "none"  
    WM_POS = parts[13] if len(parts) > 13 else "top-right"  
    WM_SIZE = parts[14] if len(parts) > 14 else "25"  
else:  
    DUMP_ID = raw_dump  
    LOGO_ID = "none"  
    CODEC = "libx264"  
...
async def encode_phase(app, video_path, sub_path, logo_path, msg_id):  
    ...
    if TASK_TYPE == "compress":  
        ...
        cmd.extend([  
            '-c:v', CODEC, '-preset', PRESET, '-crf', CRF, '-c:a', AUDIO_CODEC, '-c:s', 'copy',  
        ...
        cmd =[  
            ...
            '-c:v', 'copy', '-c:a', AUDIO_CODEC, f'-c:s', sub_codec,  
        ...
        pos_key = WM_POS  
        if pos_key == "top-left": x, y = "10", "10"  
        elif pos_key == "top-center": x, y = "(W-w)/2", "10"  
        elif pos_key == "top-right": x, y = "W-w-10", "10"  
        elif pos_key == "center-left": x, y = "10", "(H-h)/2"  
        elif pos_key == "center-right": x, y = "W-w-10", "(H-h)/2"  
        elif pos_key == "bottom-left": x, y = "10", "H-h-10"  
        elif pos_key == "bottom-center": x, y = "(W-w)/2", "H-h-10"  
        elif pos_key == "bottom-right": x, y = "W-w-10", "H-h-10"  
        else: x, y = "W-w-10", "10"  
  
        if WM_TYPE == "text" and WM_TEXT and WM_TEXT != "none":  
            text_escaped = WM_TEXT.replace("\\", "\\\\").replace(":", "\\:").replace("'", r"\'")  
            filter_complex.append(f"{current_v}drawtext=text='{text_escaped}':fontsize={WM_SIZE}:x={x}:y={y}:fontcolor=white:shadowx=2:shadowy=2[outv]")  
            current_v = "[outv]"  
        elif WM_TYPE == "image" and logo_path and LOGO_ID != "none":  
            wm_h = int(WM_SIZE) * 3  
            abs_logo = os.path.abspath(logo_path).replace('\\', '/').replace(':', '\\:')  
            filter_complex.append(f"[1:v]scale=-1:{wm_h}[logo];{current_v}[logo]overlay={x}:{y}[outv]")  
            current_v = "[outv]"  
  
        cmd =['ffmpeg', '-y', '-i', video_path]  
        if WM_TYPE == "image" and logo_path and LOGO_ID != "none":  
        ...
        cmd.extend(['-sn', '-c:v', CODEC, '-preset', PRESET, '-crf', CRF, '-c:a', AUDIO_CODEC, '-progress', 'pipe:1', output])  
